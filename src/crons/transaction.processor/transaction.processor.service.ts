...
export class TransactionProcessorService {
  ...
  async handleNewTransactions() {
    await this.transactionProcessor.start({
      ...
      onTransactionsReceived: async (shard, nonce, transactions) => {
        ...
        this.logger.log(
          `New transactions: ${transactions.length} for shard ${shard} and nonce ${nonce}`,
        );
        ...
        for (const transaction of transactions) {
          const invalidatedTokenProperties =
            await this.tryInvalidateTokenProperties(transaction);
          const invalidatedOwnerKeys = await this.tryInvalidateOwner(
            transaction,
          );
          const invalidatedCollectionPropertiesKeys =
            await this.tryInvalidateCollectionProperties(transaction);
        ...
        const distinctSendersAndReceivers = transactions
          .selectMany((transaction) => [
            transaction.sender,
            transaction.receiver,
          ])
          .distinct();
        const txCountInvalidationKeys = distinctSendersAndReceivers.map(
          (address) => CacheInfo.TxCount(address).key,
        );
      ...
      getLastProcessedNonce: async (shardId) => {
        return await this.cachingService.get<number>(
          CacheInfo.TransactionProcessorShardNonce(shardId).key,
        );
      },
      setLastProcessedNonce: async (shardId, nonce) => {
        const event = new LogMetricsEvent();
        event.args = [shardId, nonce];
        this.eventEmitter.emit(MetricsEvents.SetLastProcessedNonce, event);

        await this.cachingService.set<number>(
          CacheInfo.TransactionProcessorShardNonce(shardId).key,
          nonce,
          CacheInfo.TransactionProcessorShardNonce(shardId).ttl,
        );
      },
    });
  }

  private async tryInvalidateTokenProperties(
    transaction: ShardTransaction,
  ): Promise<string[]> {
    if (
      transaction.receiver !== this.apiConfigService.getEsdtContractAddress()
    ) {
      return [];
    }
    ...
    if (transactionFuncName === 'controlChanges') {
      ...
      if (args && args.length > 0) {
        ...
        return await this.cachingService.deleteInCache(
  ...
  async tryInvalidateOwner(transaction: ShardTransaction): Promise<string[]> {
    const transactionFuncName = transaction.getDataFunctionName();
    if (
      transactionFuncName !== 'mergeValidatorToDelegationWithWhitelist' &&
      transactionFuncName !== 'makeNewContractFromValidatorData'
    ) {
      return [];
    }

    return await this.nodeService.deleteOwnersForAddressInCache(
      transaction.sender,
    );
  }

  async tryInvalidateCollectionProperties(
    transaction: ShardTransaction,
  ): Promise<string[]> {
    if (!transaction.data) {
      return [];
    ...
    const tryExtractTransferOwnership: TransactionExtractorInterface<{
      identifier: string;
    }> = new TransferOwnershipExtractor();
    const metadataTransferOwnership =
      tryExtractTransferOwnership.extract(transaction);
    if (metadataTransferOwnership) {
      this.logger.log(
        `Detected NFT Transfer ownership for collection with identifier '${metadataTransferOwnership.identifier}'`,
      );
